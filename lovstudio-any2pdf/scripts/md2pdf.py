...
def esc_code(text):
    ...
    for line in text.split('\n'):
        e = esc(line)
        # Replace ALL spaces with &nbsp; to preserve both indentation and
        # mid-line alignment (e.g. ASCII diagrams, table output, padded columns).
...
class PDFBuilder:
    ...
    def build(self, md_text, output_path):
        ...
        # When a cover is present and the markdown has an H1 heading, strip all
        # visual elements generated by that H1 (Spacer, Paragraph, decorations,
        # and the H2's leading PageBreak) to avoid a duplicate title page.
        # The H1 ChapterMark is kept so TOC anchor links remain valid.
        if self.cfg.get("cover", True):
            ...
            if story_content and isinstance(story_content[0], ChapterMark) and story_content[0].level == 0:
                idx = 1
                # Remove everything between H1 ChapterMark and the next ChapterMark
                # (H2). This includes Spacers, Paragraphs, decorations, and the
                # H2's leading PageBreak — all in one pass.
                while idx < len(story_content) and not isinstance(story_content[idx], ChapterMark):
...
def main():
    ...
    # Parse YAML-style frontmatter (--- ... ---) from the top of the file.
    # CLI args take precedence over frontmatter values.
    frontmatter = {}
    ...
    if fm_match:
        ...
        md_text = md_text[fm_match.end():]  # strip frontmatter from body

    # Extract title from first H1 if not provided by args or frontmatter
    title = args.title or frontmatter.get("title", "")
    if not title:
        m = re.search(r'^# (.+)$', md_text, re.MULTILINE)
        title = m.group(1).strip() if m else "Document"

    theme = load_theme(args.theme if args.theme != "warm-academic" else frontmatter.get("theme", args.theme), args.theme_file)
    a = theme['accent']
    ...
    config = {
        "title": title,
        "subtitle": args.subtitle or frontmatter.get("subtitle", ""),
        "author": args.author or frontmatter.get("author", ""),
        "date": args.date or frontmatter.get("date", str(date.today())),
        "version": args.version or frontmatter.get("version", ""),
        "watermark": args.watermark or frontmatter.get("watermark", ""),
        "theme": theme,
        "accent_hex": accent_hex,
        "cover": args.cover if args.cover is not True else (frontmatter.get("cover", "true").lower() != "false"),
        "toc": args.toc if args.toc is not True else (frontmatter.get("toc", "true").lower() != "false"),
        "page_size": A4 if (args.page_size == "A4" and frontmatter.get("page-size", "A4") == "A4") else LETTER,
        "frontispiece": args.frontispiece or frontmatter.get("frontispiece", ""),
        "banner": args.banner or frontmatter.get("banner", ""),
        "header_title": args.header_title or frontmatter.get("header-title", ""),
        "footer_left": args.footer_left or frontmatter.get("footer-left", "") or args.author or frontmatter.get("author", ""),
        "stats_line": args.stats_line or frontmatter.get("stats-line", ""),
        "stats_line2": args.stats_line2 or frontmatter.get("stats-line2", ""),
        "edition_line": args.edition_line or frontmatter.get("edition-line", ""),
        "disclaimer": args.disclaimer or frontmatter.get("disclaimer", ""),
        "copyright": args.copyright or frontmatter.get("copyright", ""),
        "code_max_lines": args.code_max_lines if args.code_max_lines != 30 else int(frontmatter.get("code-max-lines", 30)),
    }
